...
module.exports = {
        ...
        exec(realm_name, details) {
            ...
            this.router.handle_methods(this.details, (err, method, extra) => {
                if (err) this.close(WAMP.ABORT, errors.AUTHORIZATION_FAILED);
                    ...
                    if (!realm.add_session(this)) return;
                    this.send([WAMP.WELCOME, this.id, {roles: meta.roles, authid: 'anonymous', authmethod: 'anonymous'}]);
                }
                else {
                    this.details.authenticating = true;
                    this.details.authmethod = method;
    ...
    [WAMP.AUTHENTICATE]: {
        ...
        exec(secret) {
            ...
            this.router.authenticate(this.details, secret, (err, details = {}) => {
                if (err) {
                ...
                if (!realm.add_session(this)) return;
                this.send([WAMP.WELCOME, this.id, {...{roles: meta.roles, authid: this.details.authid, authmethod: this.details.authmethod, authrole: this.details.authrole, authprovider: this.details.authprovider}, ...details}]);
            });
        }
    },
    [WAMP.GOODBYE]: {
    ...
    [WAMP.REGISTER]: {
        ...
        exec(request_id, options, uri) {
            this.router.authorize(this.details, 'register', uri, (err, allowed) => {
                ...
                else this.realm.create_registration(this, request_id, uri, options);
            });
        }
    },
    [WAMP.UNREGISTER]: {
    ...
    [WAMP.CALL]: {
        ...
        exec(request_id, options, uri, args, kwargs) {
            this.router.authorize(this.details, 'call', uri, (err, allowed) => {
                ...
                else this.realm.call_registration(this, request_id, uri, args, kwargs, options);
            });
        }
    },
    [WAMP.CANCEL]: {
    ...
    [WAMP.YIELD]: {
        ...
        exec(invocation_id, options, args, kwargs) {
            this.router.authorize(this.details, 'register', this.realm.invocation_uri(invocation_id), (err, allowed) => {
                ...
                else this.realm.call_result(this, invocation_id, args, kwargs, options);
            });
        }
    },
    [WAMP.ERROR]: {
    ...
    [WAMP.SUBSCRIBE]: {
        ...
        exec(request_id, options, uri) {
            this.router.authorize(this.details, 'subscribe', uri, (err, allowed) => {
                ...
                else this.realm.create_subscription(this, request_id, uri, options);
            });
        }
    },
    [WAMP.UNSUBSCRIBE]: {
    ...
    [WAMP.PUBLISH]: {
        ...
        exec(request_id, options, uri, args, kwargs) {
            this.router.authorize(this.details, 'publish', uri, (err, allowed) => {
                ...
                else this.realm.publish(this, request_id, uri, args, kwargs, options);
            });
        }
    },
};
