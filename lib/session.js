...
function Session (router, wsclient) {
    ...
    this.handle = function(message) {
        ...
        switch (message_type) {
            ...
            case WAMP.GOODBYE: {
                if (this.realm === null) {
                    this.logger.error("Session ", this.id, ", authid ", this.secure_details.authid, " goodbyes but it never joined realm");
                    this.close(WAMP.ABORT, errors.PROTOCOL_VIOLATION);
                    return;
                }
            ...
            case WAMP.UNREGISTER: {
                ...
                this.router.authorize(this.secure_details, "register", this.realm.registration_uri(registration_id), (err, allowed) => {
                    ...
                    if (!this.realm.registrations_by_uri[registration.uri].callees.length) delete this.realm.registrations_by_uri[registration.uri];
            ...
            case WAMP.SUBSCRIBE: {
                ...
                this.router.authorize(this.secure_details, "subscribe", uri, (err, allowed) => {
                    ...
                    if (!this.realm.subscriptions_by_uri.hasOwnProperty(uri)) this.realm.subscriptions_by_uri[uri] = {subscribers: [], subscriber_ids: {}};
                    if (this.realm.subscriptions_by_uri[uri].subscriber_ids[this.id]) {
                        this.logger.warn("Session ", this.id, ", authid ", this.secure_details.authid, " already subscribed to topic ", uri, ". Sending same subscription ID");
                        this.send([WAMP.SUBSCRIBED, request_id, this.realm.subscriptions_by_uri[uri].subscriber_ids[this.id].id]);
                        return;
                    }
                    let subscription = {session_id: this.id, request_id, uri, options, id: id()};
                    this.realm.subscriptions_by_uri[uri].subscriber_ids[this.id] = subscription;
                    this.realm.subscriptions_by_uri[uri].subscribers.push(subscription);
            ...
            case WAMP.UNSUBSCRIBE: {
                ...
                this.router.authorize(this.secure_details, "subscribe", this.realm.subscription_uri(subscription_id), (err, allowed) => {
                    ...
                    this.realm.subscriptions_by_uri[subscription.uri].subscribers = this.realm.subscriptions_by_uri[subscription.uri].subscribers.filter(subscription => subscription.session_id !== this.id);
                    delete this.realm.subscriptions_by_uri[subscription.uri].subscriber_ids[this.id];
                    if (!this.realm.subscriptions_by_uri[subscription.uri].subscribers.length) delete this.realm.subscriptions_by_uri[subscription.uri];
            ...
            case WAMP.PUBLISH: {
                ...
                this.router.authorize(this.secure_details, "publish", uri, (err, allowed) => {
                    ...
                    if (this.realm.subscriptions_by_uri[uri] && this.realm.subscriptions_by_uri[uri].subscribers.length) {
                        this.realm.subscriptions_by_uri[uri].subscribers.forEach(subscription => {
