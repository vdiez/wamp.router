...
class Realm {
    ...
    create_registration(session, request_id, options = {}, uri) {
        ...
        const registration = this.registrations_by_uri[options.match][uri] || {callees: [], callee_ids: {}, round_robin: 0, id: registration_id, uri, options, created: now};
        ...
        const callee = {id: session.id, created: now, request_id, options, counter: 0, registration_id};
    ...
    call_registration(session, call_id, options, uri, args, kwargs) {
    ...
    create_subscription(session, request_id, options, uri) {
        ...
        if (!options.match || !meta.allowed_match.includes(options.match)) options.match = 'exact';
        const subscription = this.subscriptions_by_uri[options.match][uri] || {subscribers: [], subscriber_ids: {}, id: subscription_id, uri, options, created: now};
        ...
        const subscriber = {id: session.id, created: now, request_id, uri, options, subscription_id};
