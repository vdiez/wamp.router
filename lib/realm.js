...
class Realm {
    ...
    remove_session(session) {
        if (this.sessions.hasOwnProperty(session.id)) {
            ...
            for (let id in this.invocations) {
                if (this.invocations.hasOwnProperty(id)) {
                    if (this.invocations[id].callee_id === session.id) {
                        let caller = this.sessions[this.invocations[id].session_id];
                        if (caller) caller.cancel_call(this.invocations[id].request_id);
                    }
            ...
            this.logger.warn("Session ", session.id, ", authid ", session.secure_details.authid, " left realm ", this.name);
        }
    }
}
