...
class Realm {
    ...
    create_registration(session, request_id, options = {}, uri) {
        ...
        const registrations = this.registrations_by_uri[uri] || {callees: [], callee_ids: {}, round_robin: 0};
        ...
        if (!this.registrations_by_uri[uri]) this.registrations_by_uri[uri] = registrations;
        const registration = {session_id: session.id, created: new Date().toISOString(), request_id, uri, options, counter: 0, id: id()};
    ...
    match_registrations(uri, forbidden = [], force_matching = 'any') {
        const registrations = this.registrations_by_uri[uri];
        let callees = registrations?.callees || [];
        if (force_matching === 'any' || force_matching === 'exact') {
            if (forbidden?.length) callees = callees.filter(registration => !forbidden.includes(registration.session_id));
            callees = callees.filter(registration => registration.options.match === 'exact');
        }

        if (!callees?.length || force_matching === 'wildcard' || force_matching === 'prefix') {
            if (force_matching === 'exact') return;
