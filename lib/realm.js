...
class Realm {
        ...
        if (match === 'exact') {
            ...
            for (const registered_uri in this.subscriptions_by_uri) {
                if (this.subscriptions_by_uri.hasOwnProperty(registered_uri) && match_functions[match](uri, registered_uri)) {
                    const subscription = this.subscriptions_by_uri[registered_uri];
                    if (only_subscriptions) result.push(subscription);
                    else {
                        for (let i = 0; i < subscription.subscribers.length; i++) {
                            if (subscription.subscribers[i].options.match === match) {
    ...
    publish(session, request_id, options, uri, args, kwargs) {
        ...
        subscribers.forEach(subscriber => {
            const subscriber_session = this.sessions[subscriber.session_id];
            if (!subscriber_session) return;
            if (subscriber.session_id === session.id && options.exclude_me !== false) return;
            if (options.exclude?.includes?.(subscriber.session_id)) return;
            ...
            if (options.eligible?.length && !options.eligible.includes(subscriber.session_id)) return;
